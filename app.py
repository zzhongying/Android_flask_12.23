...
def get_bestParams():
    ...
    data = {"legend_data": ["推荐参数", u"实际参数"],
            "yAxis_data": ["eta", "max_depth", "min_child_weight", "subsample", "colsample_bytree", "gamma", "lambda", "alpha"],
...
def get_dataInfo(data_name=None):
    ...
    try:
        ...
        total_lines = -1
    info['total_lines'] = total_lines    # 文件行数
    info['num_feature'] = basicData.test_x.shape[1]  # 属性数量

    # 标签分类情况
    # df = pd.read_csv(filepath, usecols=['Class']).value_counts()
    df = pd.read_csv(filepath)['Class'].value_counts()
    tags = {key[0]: int(value) for key, value in zip(df.index, df.values)}
    info['tags'] = tags
...
def change_feature():
    ...
    feature_route = [{'source': 'train'+f_name, 'target': 'test'+f_name} for f_name in train_feature if f_name in test_feature]
